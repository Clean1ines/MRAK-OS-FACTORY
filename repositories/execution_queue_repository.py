...
async def enqueue(node_execution_id: str, tx=None) -> str:
    ...
    try:
        ...
        await conn.execute("""
            INSERT INTO execution_queue (id, node_execution_id, status, created_at, updated_at)
            VALUES ($1, $2, 'PENDING', NOW(), NOW())
        """, job_id, node_execution_id)
        return job_id
    finally:
        if close_conn:
            await conn.close()
