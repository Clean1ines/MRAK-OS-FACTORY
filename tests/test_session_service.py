...
async def test_create_clarification_session(session_service):
    with patch("session_service.db.create_clarification_session", new_callable=AsyncMock) as mock_create:
...
async def test_get_clarification_session(session_service):
    with patch("session_service.db.get_clarification_session", new_callable=AsyncMock) as mock_get:
...
async def test_add_message_to_session(session_service):
    with patch("session_service.db.add_message_to_session", new_callable=AsyncMock) as mock_add:
...
async def test_list_active_sessions_for_project(session_service):
    with patch("session_service.db.list_active_sessions_for_project", new_callable=AsyncMock) as mock_list:
